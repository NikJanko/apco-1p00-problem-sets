...
class Hint_Manager:
    def __init__(self):
        self.hint_dict = {
            1: ["You require a path to read the file, the path is: FILES_DIR/data_duo.txt", "Reading the lines returns a list of strings. You'll need to convert each string to an integer.", "use 'with open(PATH, 'r') as f:' to read the file, and then 'lines = f.readlines()' to get the lines of the file."],
            2: ["The path to read the file is: FILES_DIR/data_tri.txt", "The product (or multiplication) is done via the * (asterisk/star) operator.", "use 'with open(PATH, 'r') as f:' to read the file, and then 'lines = f.readlines()' to get the lines of the file."],
            3: ["The path to read the file is: FILES_DIR/data_solo.txt", "The factorial of a number n is the product of all positive integers less than or equal to n. Since we are doing repetition, use a loop.", "use 'with open(PATH, 'r') as f:' to read the file, and then 'lines = f.readlines()' to get the lines of the file."],
            4: ["The path to read the file is: FILES_DIR/grades.csv", "you will need to import the csv module (WITHIN THE FUNCTION SCOPE).", "You should read and save all the contents of the file in a variable before writing the contents to a new file.", "use 'with open(PATH, 'r') as f:' and csv.reader() to read the file, use 'with open(PATH, 'w', newline='') as f:' and csv.writer() to write the file, and remember to calculate the average grade for each student. before writing."],
            5: ["The path to write the file is: GENERATED_DIR/output.csv", "You need to create a CSV writer object using csv.writer().", "use 'with open(PATH, 'w', newline='') as f:' to write the file, and then use writer.writerow() to write each row."],
            6: ["The path to read the file is: FILES_DIR/data_duo.txt", "The difference of two integers a and b can be calculated as a - b.", "use 'with open(PATH, 'r') as f:' to read the file, and then 'lines = f.readlines()' to get the lines of the file."],
            7: ["Import the random module, and use random.seed(10), make sure the import is within the function.", "Use random.sample() [LOOK AT THE DOCUMENTATION] to generate unique random integers within a specified range.", "use 'with open(PATH, 'w') as f:' to write the file, and remember to convert the integers to strings before writing them to the file. The PATH is GENERATED_DIR/recur.txt"],
            8: ["The path to read the file is: GENERATED_DIR/recur.txt", "use 'with open(PATH, 'r') as f:' to read the file, and then 'lines = f.readlines()'", "Psudeo code: \nread the file,\n if the length of the array is less than or equal to 1: return the array,\n otherwise: take the center of the array as a pivot,\n take a slice of the left side of the array as everything smaller than the pivot,\n take a slice of the right side of the array as everything bigger than the pivot,\n take a slice of the array which only holds every instance of the pivot,\n return the result of recursively calling the function on the left slice, then the pivot_array, then recursively calling the function on the right slice.", "(recursive steps solution):\n\nif len(array) <= 1:\n    return array\npivot = array[len(array) // 2]\nleft = []\nright = []\npivot_array = []\nfor num in array:\n    if num < pivot:\n        left.append(num)\n    elif num > pivot:\n        right.append(num)\n    else:\n        pivot_array.append(num)\nreturn quicksort(left) + pivot_array + quicksort(right)"],
            9: ["The path to read the file is: GENERATED_DIR/recur.txt", "use 'with open(PATH, 'r') as f:' to read the file, and then 'lines = f.readlines()' to get the lines of the file.", "Pseudo code: \nread the file,\n set low to 0 and high to the length of the array - 1,\n if low is greater than high, return -1 (base case for not found),\n find the middle index as (low + high) // 2,\n if the target is less than the middle element, recursively search the left half of the array (high becomes mid - 1),\n if the target is greater than the middle element, recursively search the right half of the array (low becomes mid + 1),\n if the target is equal to the middle element, return the middle index.", "(recursive steps solution):\n\nlow = 0\nhigh = len(array) - 1\nif low > high:\n    return -1\nmid = (low + high) // 2\nif target < array[mid]:\n    return binary_search(array, target, low, mid - 1)\nelif target > array[mid]:\n    return binary_search(array, target, mid + 1, high)\nelse:\n    return mid"],
            ...
            18: ["the file to read is: FILES_DIR/cipher_passage.txt", "use 'with open(PATH, 'r') as f:' to read the file, and then 'lines = f.readlines()' to get the lines of the file.", "loop through the characters, replacing each read character with the character in the mapping dictionary via lookup"],
